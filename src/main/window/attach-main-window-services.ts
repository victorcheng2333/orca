...
import { dismissAvailableUpdate, dismissNudge, type UpdateInstallMode } from '../updater'
...
export function registerUpdaterHandlers(_store: Store): void {
  ...
  const rejectOfficialUpdate = (): never => {
    throw new Error(OFFICIAL_UPDATES_DISABLED_MESSAGE)
  }

  ipcMain.handle('updater:getStatus', () => ({ state: 'not-available' }))
  ipcMain.handle('updater:getVersion', () => app.getVersion())
  ipcMain.handle('updater:check', rejectOfficialUpdate)
  ipcMain.handle('updater:download', rejectOfficialUpdate)
  ipcMain.handle('updater:quitAndInstall', rejectOfficialUpdate)
  ...
  ipcMain.handle('updater:getLinuxPackageInstallInstructions', (event) => {
    ...
    return rejectOfficialUpdate()
  })
  ipcMain.handle('updater:showLinuxPackage', (event) => {
    assertTrustedUpdaterRecoverySender(event)
    return rejectOfficialUpdate()
  })
  ipcMain.handle('updater:listBuilds', rejectOfficialUpdate)
}
