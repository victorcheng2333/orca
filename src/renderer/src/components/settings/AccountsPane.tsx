        ...
        description={
          accountRuntime.runtime === 'wsl' && !wslAvailable && !wslCapabilitiesLoading
            ...
            : translate(
                ...
                'Choose which local environment to inspect and where new managed Codex accounts are added.'
              )
        }
        control={
      ...
      <section key="claude-accounts" id="accounts-claude" className="space-y-4 scroll-mt-6">
          ...
          <p className="text-xs text-muted-foreground">
            {CLAUDE_MANAGED_ACCOUNTS_ENABLED
              ? translate(
                  ...
                  'Optional managed profiles for distinct Claude identities.'
                )
              : translate(
                  'auto.components.settings.AccountsPane.systemClaudeAuthOnly',
                  'Orca uses the system Claude Code sign-in and does not store or switch Claude subscription credentials.'
                )}
          </p>
        </div>
        ...
        {CLAUDE_MANAGED_ACCOUNTS_ENABLED ? (
          <SearchableSetting
            ...
            keywords={['claude', 'account', 'authentication']}
          ...
          <SearchableSetting
            title={translate(
              'auto.components.settings.AccountsPane.systemClaudeAuthTitle',
              'Claude Code Authentication'
            )}
            description={translate(
              'auto.components.settings.AccountsPane.systemClaudeAuthDescription',
              'Sign in, refresh credentials, and sign out through the official Claude Code client.'
            )}
            keywords={['claude', 'account', 'authentication', 'login']}
            className="py-2"
          >
            <p className="text-xs text-muted-foreground">
              {translate(
                'auto.components.settings.AccountsPane.systemClaudeAuthInstruction',
                'Run Claude Code in a terminal and use its /login command to change the system account.'
              )}
            </p>
          </SearchableSetting>
        )}
